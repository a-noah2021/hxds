...
import static com.example.hxds.common.wxpay.WXPayConstants.SignType;
...
public class WXPay {
    ...
    public boolean isResponseSignatureValid(Map<String, String> reqData,SignType signType) throws Exception {
        // 返回数据的签名方式和请求中给定的签名方式是一致的
    ...
    public Map<String, String> processResponseXml(String xmlStr,SignType signType) throws Exception {
        String RETURN_CODE = "return_code";
        String return_code;
        Map<String, String> respData = WXPayUtil.xmlToMap(xmlStr);
        if (respData.containsKey(RETURN_CODE)) {
        ...
        if (return_code.equals(WXPayConstants.FAIL)) {
            ...
            if (this.isResponseSignatureValid(respData,signType)) {
                return respData;
            } else {
                throw new Exception(String.format("Invalid sign value in XML: %s", xmlStr));
            }
